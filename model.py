...
class CustomLLM(nn.Module):
    ...
    def generate(
        ...
        """
        Generates text using various decoding strategies with improved stability.
        """
        # Ensure model is in eval mode
        self.eval()
        
        # Move inputs to model device
        input_ids = input_ids.to(self.token_embeddings.weight.device)
        ...
        for _ in range(max_new_tokens):
            ...
            if torch.isnan(next_token_logits).any() or torch.isinf(next_token_logits).any():
                print("⚠️ Detected NaN or Inf in logits!")
            ...
            if repetition_penalty != 1.0:
                ...
                score = torch.where(score < 0, score * repetition_penalty, score / (repetition_penalty + 1e-6))
                next_token_logits.scatter_(1, input_ids, score)
                
                # Debugging
                if torch.isnan(next_token_logits).any() or torch.isinf(next_token_logits).any():
                    print("⚠️ NaN or Inf detected after applying repetition penalty!")
            ...
            if top_k is not None and top_k > 0:
                ...
                if torch.all(next_token_logits == -float('inf')):
                    next_token_logits[torch.argmax(next_token_logits, dim=-1)] = 0
